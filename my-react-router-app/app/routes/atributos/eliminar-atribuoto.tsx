...
export default function EliminarAtributo() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>): Promise<void> {
    ...
    const trimmedId = idAtributo?.trim();
    if (!trimmedId) {
      setFeedbackMessage("No se recibió un IdAtributo.");
      return;
    }
    ...
    try {
      const response = await fetch(
        `/api/Atributos/DeleteAtributo/IdAtributo/${encodeURIComponent(trimmedId)}`,
        {
          method: "DELETE",
      ...
      // si querés redirigir después de eliminar:
      // navigate("/atributos");
    } catch (error) {
  ...
  return (
    <div style={{ padding: 16, maxWidth: 480 }}>
      ...
      >
        ...
        {feedbackMessage && <div style={{ marginTop: 8 }}>{feedbackMessage}</div>}
      </form>
    </div>
  );
}
